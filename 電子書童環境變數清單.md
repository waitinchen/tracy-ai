...
**開發方式**: **Supabase Local Development（Docker-based）**  
**功能覆蓋**: ≈ 雲端 Supabase 90%

| 變數名稱 | 類型 | 說明 | 本地開發值 | 雲端值 |
|---------|------|------|-----------|--------|
| `NEXT_PUBLIC_SUPABASE_URL` | **必需** | Supabase 專案 URL | `http://localhost:54321` | `https://xxx.supabase.co` |
| `NEXT_PUBLIC_SUPABASE_ANON_KEY` | **必需** | Supabase 匿名金鑰（公開） | 從 `supabase status` 取得 | 從 Dashboard 取得 |
| `NEXT_PUBLIC_SITE_URL` | **必需** | 網站部署 URL | `http://localhost:3000` | `https://your-domain.com` |
...
# 或使用 Scoop: scoop install supabase
```
...
**詳細設定指南**: 見 `megan/docs/supabase-local-setup.md`
...
# --- Supabase Local Development 配置（必需）---
# 取得方式: 執行 supabase start 後，執行 supabase status
NEXT_PUBLIC_SUPABASE_URL=http://localhost:54321
NEXT_PUBLIC_SUPABASE_ANON_KEY=你的_anon_key_從_supabase_status_取得
...
**重要**: 
- 本地開發使用 `http://localhost:54321`（Supabase Local）
- 取得 Anon Key: 執行 `supabase status` 後複製 `anon key`
- 詳細設定見 `megan/docs/supabase-local-setup.md`
...
```env
# 最小配置 - 本地開發（Supabase Local）
NEXT_PUBLIC_SUPABASE_URL=http://localhost:54321
NEXT_PUBLIC_SUPABASE_ANON_KEY=從_supabase_status_取得
NEXT_PUBLIC_SITE_URL=http://localhost:3000
GOOGLE_API_KEY=xxx
...
- `megan/docs/supabase-local-setup.md` - **Supabase Local Development 完整設定指南**（推薦）
